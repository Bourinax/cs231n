...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    h_activ = X.dot(W1) + b1
    h_activ[h_activ < 0] = 0
    hidden_size = np.shape(h_activ)[1]
    #print("shape h_activ: " + str(np.shape(h_activ)))
    scores = h_activ.dot(W2) + b2
    output_size = np.shape(scores)[1] # check that
    ...
    # first computing the grad for W2
    ind1 = np.indices((output_size, num_train))[0]
    #print("shape ind1: " + str(np.shape(ind1)))
    ind2 = np.repeat(y[np.newaxis,:], output_size, axis = 0)
    ...
    # now computing b2
    #print("shape truc: " + str(np.shape(truc)))
    #print("shape np.ones(output_size): " + str(np.shape(np.ones(output_size))))
    db2 = np.ones([1,num_train]).dot(dscores)
    grads['b2'] = db2
    ...
    # now computing W1
    #dW1 = W1
    dW1 = ((dhidden.T).dot(X)).T
    # gradient regularisation
    dW1 += 2*reg*W1
    grads['W1'] = dW1
    #print("shape dW1: " + str(np.shape(dW1)))
    # now computing b1
    db1 = np.ones([1,num_train]).dot(dhidden)
    grads['b1'] = db1
